...
// Function to get VAPI configuration with ElevenLabs agent ID
const getVapiConfig = (practiceConfig: any) => ({
  publicKey: "your-vapi-public-key", // You need to get this from VAPI dashboard
  assistant: {
    firstMessage: practiceConfig.voice.firstMessage,
    model: {
      provider: "openai",
      model: "gpt-4",
      messages: [
    ...
    voice: {
      provider: "elevenlabs",
      voiceId: practiceConfig.agentId,
...
export default function ThaiVoiceAgent() {
  ...
  useEffect(() => {
    ...
    const initVapi = async () => {
      try {
        // For now, we'll use a direct ElevenLabs implementation
        // since we don't have VAPI keys yet
        console.log('Initializing Thai Voice Agent with ElevenLabs');
      } catch (error) {
  ...
  const startConversation = async () => {
    ...
    try {
      // Direct ElevenLabs API call for Thai voice
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-gray-600">
          Real-time conversational AI with ElevenLabs Thai Voice
        </p>
      ...
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 rounded-lg border border-purple-200">
        ...
        <div className="mt-4 p-3 bg-purple-100 rounded-lg">
          <p className="text-xs text-purple-600">
            <strong>หมายเหตุ:</strong> ใช้ ElevenLabs Thai Voice Agent สำหรับการสนทนาภาษาไทยที่เป็นธรรมชาติ
          </p>
        </div>
      </div>
    </div>
